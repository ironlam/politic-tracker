...
export async function GET() {
  ...
  try {
    ...
    console.error("GET /api/admin/affaires/moderate error:", error);
    return NextResponse.json(
      { error: "Erreur lors de la récupération des revues de modération" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("POST /api/admin/affaires/moderate error:", error);
    return NextResponse.json(
      { error: "Erreur lors de l'application de la modération" },
      { status: 500 }
    );
  }
}
